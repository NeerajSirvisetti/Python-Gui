...
class ApicGui:
    def __init__(self, root, steps, folder_path ):
        ...
        for folder in [self.dbs_folder, self.rpt_folder, self.summary_folder, self.backup_folder]:
    ...
    def backup_folder_contents(self, step, source_folder):
        ...
        source_files = [f for f in os.listdir(source_folder) if f.startswith(source_pattern)]
    ...
    def show_report_files(self, selected_step):
        ...
        if report_files:
          ...
          self.display_message(f"Report Files have not been generated for this {selected_step}.", self.message_window_r2)

    def get_report_files(self, selected_step):
        report_files_pattern = f"{selected_step}*"
        return [f for f in os.listdir(self.rpt_folder) if f.startswith(report_files_pattern)]

    def display_report_files(self, report_files):
        self.display_message("Report Files:", self.message_window_r2, disappear=False, clear=True)
        for report_file in report_files:
          file_link = tk.Label(self.r2, text=report_file, fg="blue", cursor="hand2")
          file_link.pack()
          file_link.bind("<Button-1>", lambda event, file=report_file: self.open_file_contents(file))

    def open_file_contents(self, file_name):
        file_path = os.path.join(self.rpt_folder, file_name)
        with open(file_path, 'r') as file:
          contents = file.read()
          self.display_message(contents, self.message_window_s2, disappear=False, clear=True)
    ...
    def show_summary_content(self, selected_step):
        ...
        if summary_file:
          ...
          self.display_message(f"Summary File has not been generated for this {selected_step}.", self.message_window_s2)

    def get_summary_file(self, selected_step):
        return f"{selected_step}.rpt"
    ...
    def ask_question(self,message):
      ...
      if response:
          ...
          if messagebox.askyesno("Backup Confirmation", f"Do you want to backup files for steps after '{selected_step}'?"):
